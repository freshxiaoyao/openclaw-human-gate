...
export function underDirectory(stored, candidate) {
    if (stored.kind !== candidate.kind || stored.volume !== candidate.volume)
        return false;
...
export function normalizeTrustedRoot(raw) {
...
function nearestRoot(dir, roots) {
    let best;
...
function remapDirectories(scope, roots) {
    const unique = new Map();
    for (const dir of scope.directories) {
        const mapped = nearestRoot(dir, roots) ?? dir;
        unique.set(directoryIdentity(mapped), mapped);
    }
    const directories = [...unique.values()].sort((a, b) => directoryIdentity(a).localeCompare(directoryIdentity(b), "en"));
    return { directories };
}
...
export function createAuthorizationFingerprint(context, options) {
    ...
    const effectivePathScope = options.pathMode === "root" && pathScope && options.writeRoots?.length
