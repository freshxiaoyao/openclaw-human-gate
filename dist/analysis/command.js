...
function packageManagerScript(args) {
    ...
    if (first === "run" || first === "run-script" || first === "exec")
        index = 1;
...
function classifyIntent(command, args) {
    if (command === "git") {
    ...
    if (command === "vite")
        return lower.includes("build") ? "build" : undefined;
    if (command === "cargo") {
        if (lower[0] === "build")
            return "build";
        if (lower[0] === "test")
    ...
    if (command === "gradle" || command === "mvn" || command === "maven" || command === "sbt") {
        ...
        if (lower[0] === "test")
            return "test";
        if (lower[0] === "compile" || lower[0] === "package" || lower[0] === "install")
            return "build";
        return undefined;
    }
    ...
    if (command === "rustfmt" || command === "black" || command === "isort")
        return "format";
    if (command === "prettier")
        return lower.includes("--write") || lower.includes("-w") ? "format" : undefined;
    if (command === "eslint")
        return lower.includes("--fix") ? "format" : undefined;
    if (command === "gofmt" || command === "gofumpt")
        return lower.includes("-w") ? "format" : undefined;
    if (command === "clang-format")
        return lower.includes("-i") ? "format" : undefined;
    return undefined;
}
...
function anyDevIntent(scans, intent) {
