 ...
 * command parameters.
 */
import { DESTRUCTIVE_NAME_TOKENS, DESTRUCTIVE_TOOL_KINDS, READONLY_NAME_TOKENS, READONLY_TOOL_KINDS, } from "./types.js";
import { tokenizeName } from "./policy.js";
import { MUTATION_PATH_KEYS, MUTATION_PATCH_KEYS, MUTATION_TOOL_NAMES, } from "./analysis/file-mutation.js";
export const SELF_PROTECTION_VERSION = 1;
/** Exec-like tools: scan their command parameters. */
const EXEC_TOOLS = new Set(["exec", "process", "code_mode_exec"]);
...
const PATCH_TOOLS = new Set([MUTATION_TOOL_NAMES.applyPatch, "applypatch"]);
...
function relevantParams(toolName, toolKind) {
...
function matchMarker(value) {
    if (DIR_MARKER_RE.test(value))
        return ".openclaw";
    if (CONFIG_MARKER_RE.test(value))
...
export function classifySensitiveEscalation(toolName, toolKind, params) {
    ...
    for (const key of keys) {
        ...
        if (typeof value !== "string" || value.length === 0 || value.length > MAX_SCAN_LENGTH)
            continue;
        const marker = matchMarker(value);
        if (marker)
            hits.push({ marker, param: key });
...
export function isReadOnlyCarrier(toolName, toolKind) {
    if (toolKind) {
    ...
    if (segments.some((seg) => DESTRUCTIVE_NAME_TOKENS.includes(seg)))
        return false;
    if (segments.some((seg) => READONLY_NAME_TOKENS.includes(seg)))
        return true;
