...
import { AUTHORIZATION_FINGERPRINT_VERSION, isAuthorizationFingerprint, } from "./scope.js";
...
function normalizeEntry(key, value) {
    ...
    const entry = value;
    if (entry.scopeKey !== key || !isScope(entry.scope) ||
...
export function normalizeWindowState(value) {
    ...
    if (candidate.version !== WINDOW_STATE_VERSION || !candidate.windows ||
...
function matchesFingerprint(entry, fingerprint) {
    return (entry.scopeKey === fingerprint.windowKey &&
...
function cleanAndBound(state, now, incomingKey) {
    const windows = {};
    for (const [key, entry] of Object.entries(state.windows)) {
        if (entry.mode === "time" && entry.expiresAt <= now)
            continue;
        windows[key] = entry;
    }
    ...
    const ordered = Object.entries(windows).sort(([keyA, a], [keyB, b]) => a.openedAt - b.openedAt || keyA.localeCompare(keyB, "en"));
...
export class ApprovalWindowStore {
    ...
    isOpen(cfg, sessionKey, fingerprint, runId, now) {
        if (cfg.mode === "off" || !isAuthorizationFingerprint(fingerprint) ||
            !Number.isFinite(now) || now < 0) {
            return false;
        }
        const entry = normalizeWindowState(this.read(sessionKey)).windows[fingerprint.windowKey];
        if (!entry || !matchesFingerprint(entry, fingerprint) || entry.mode !== cfg.mode)
            return false;
        // Clock rollback must not extend a grant into time before it was opened.
        if (now < entry.openedAt)
            return false;
        if (cfg.mode === "turn") {
    ...
    async open(cfg, sessionKey, fingerprint, runId, now) {
        if (cfg.mode === "off" || !isAuthorizationFingerprint(fingerprint) ||
            !Number.isFinite(now) || now < 0) {
            return false;
        }
        if (cfg.mode === "turn" && (!runId || runId.length === 0))
            return false;
        if (cfg.mode === "time" &&
