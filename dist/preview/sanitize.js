...
export function headTailText(value, max) {
    if (value.length <= max)
        return value;
    const separator = "\n…\n";
...
export function sanitizeText(value, redactSecrets) {
    let clean = value
        .replace(ANSI_ESCAPE, "")
    ...
    if (redactSecrets) {
        clean = clean
            ...
            .replace(SECRET_FLAG, (_match, prefix, name, separator) => `${prefix}${name}${separator}[REDACTED]`)
            .replace(COOKIE_HEADER, "Cookie: [REDACTED]")
            .replace(URL_CREDENTIALS, "$1[REDACTED]@[HOST]");
    }
    return clean;
}
export function boundedLines(value, maxLines, maxChars) {
