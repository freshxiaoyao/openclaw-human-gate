...
const PATCH_KEYS = ["input", "patch", "patchText", "patch_text"] as const;
const WRITE_PAYLOAD_KEYS = ["content", "text", "newContent", "new_content", "data"] as const;
...
function allowedEnvelopeKeys(toolName: string): ReadonlySet<string> {
...
function hasTrustedEnvelope(
  params: Readonly<Record<string, unknown>>,
  allowedKeys: ReadonlySet<string>,
): boolean {
  const prototype = Object.getPrototypeOf(params) as object | null;
...
function uniqueTargets(targets: readonly VerifiedTarget[]): VerifiedTarget[] {
  const seen = new Set<string>();
  const result: VerifiedTarget[] = [];
...
function extractDirectPath(params: Readonly<Record<string, unknown>>): Extraction {
  const candidates: VerifiedTarget[] = [];
  let invalid = false;
...
function extractPatchText(params: Readonly<Record<string, unknown>>): {
  key?: string;
  value?: string;
  complete: boolean;
} {
  ...
  const distinct = new Map(values.map((item) => [item.value, item] as const));
  if (invalid || distinct.size !== 1) return { complete: false };
  const selected = [...distinct.values()][0];
  return { key: selected?.key, value: selected?.value, complete: selected !== undefined };
}

function extractPatchTargets(
  params: Readonly<Record<string, unknown>>,
  maxLength: number,
): Extraction {
  ...
  let currentOperation: "Add" | "Update" | "Delete" | undefined;
  ...
  for (let index = 1; index < lines.length - 1; index += 1) {
    ...
    if (file) {
      currentOperation = file[1] as "Add" | "Update" | "Delete";
      if (currentOperation === "Delete") destructive = true;
      const path = file[2];
      if (!validPath(path)) {
        malformed = true;
      } else {
...
export class FileMutationAnalyzer implements ToolCallAnalyzer {
  ...
  analyze(context: ToolCallContext): AnalysisResult {
    ...
    const extraction: Extraction = envelopeTrusted
      ? extracted
      : {
    ...
    const primary = finding(
      "file-mutation.local-write",
      "filesystem",
      "Filesystem content will be modified",
      "This tool writes, edits, or removes local file content.",
      extraction.targets[0]?.path,
    );
    const findings = [primary];
    if (extraction.destructive) {
      findings.push(finding(
        "file-mutation.delete-or-move",
        "filesystem",
        "Patch deletes or moves filesystem content",
        "Delete and move operations use a separate destructive semantic scope.",
        extraction.targets[0]?.path,
      ));
    }
    if (!extraction.complete) {
      findings.unshift(finding(
        "file-mutation.target-unverified",
        "unknown",
        "Filesystem target is not completely verified",
        extraction.problem ?? "The authoritative input did not yield a complete target set.",
      ));
    }
    ...
    return {
      ...
      effects: extraction.complete
        ? ["local-write", ...(extraction.destructive ? ["destructive" as const] : [])]
        : ["local-write", "unknown"],
