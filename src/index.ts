...
function formatLeaseRemaining(ms: number): string {
...
const pluginEntry: OpenClawPluginDefinition = definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    const denyCooldown = new DenyCooldownStore(
      (sessionKey) =>
        normalizeDenyCooldownState(extensionValue(api, sessionKey, DENY_COOLDOWN_NAMESPACE)),
      (sessionKey, update) =>
        patchExtension(
          api,
          sessionKey,
          DENY_COOLDOWN_NAMESPACE,
          { version: DENY_COOLDOWN_STATE_VERSION, denials: {} },
          normalizeDenyCooldownState,
          update,
        ),
      config.denyCooldownMs,
    );
    const execLease = new ExecLeaseStore(
      (sessionKey) =>
        normalizeExecLeaseState(extensionValue(api, sessionKey, EXEC_LEASE_NAMESPACE)),
      (sessionKey, update) =>
        patchExtension(
          api,
          sessionKey,
          EXEC_LEASE_NAMESPACE,
