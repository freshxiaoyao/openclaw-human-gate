...
export const READONLY_NAME_PATTERN =
  /^(read|get|list|search|glob|grep|view|show|status|ping|fetch|head|cat|ls|find|whoami|echo|inspect|describe|explain|query|count)[_-a-zA-Z0-9]*$/i;

export const DESTRUCTIVE_NAME_PATTERN =
  /^(write|edit|delete|remove|rm|rmdir|mkdir|move|rename|deploy|publish|install|uninstall|exec|run|apply|patch|create|update|kill|send|post|put|push|commit|flush|drop|truncate|grant|revoke)[_-a-zA-Z0-9]*$/i;
