...
function containsToken(name: string, tokens: readonly string[]): boolean {
...
function conditionMatches(
  ...
  if (Object.prototype.hasOwnProperty.call(condition, "matches")) {
    ...
    const source = ownDataValue(condition, "matches") as string;
    try {
...
function classifyByName(
  ...
  if (containsToken(toolName, DESTRUCTIVE_NAME_TOKENS)) {
    return requireApprovalDecision(
      ...
      `Destructive name token in "${toolName}"`,
      config,
    );
  }
 ...
 *  standalone segment.
 */
export function isAutoPassContext(
  sessionKey: string,
